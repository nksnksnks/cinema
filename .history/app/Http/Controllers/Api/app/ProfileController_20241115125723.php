    ...
    public function updateOrCreate(ProfileRequest $request)
{
    try {
        ...
        // Lưu public_id của ảnh cũ nếu có
        $oldAvatar = $profile ? $profile->avatar : null;
        $oldAvatarPublicId = null;

        // Nếu có avatar cũ, lấy public_id (nếu có)
        if ($oldAvatar) {
            $oldAvatarPublicId = cloudinary()->getPublicId($oldAvatar);
        }
        ...
        if ($request->hasFile('avatar')) {
            ...
            if ($oldAvatarPublicId) {
                cloudinary()->destroy($oldAvatarPublicId); // Xóa ảnh cũ theo public_id
            }
        }
