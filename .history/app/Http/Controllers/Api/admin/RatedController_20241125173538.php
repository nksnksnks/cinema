...
class RatedController extends Controller
{
    ...
    public function countryStore(RatedRequest $request)
    {
        try {
            DB::beginTransaction();
            $data = $request->all();
    ...
    public function countryEdit(string $id){
        ...
        $config['method'] = 'edit';
        return view('admin.rated.create', compact('config','country'));
    }
  
    public function countryUpdate($id, RatedRequest $request){
        try {
            DB::beginTransaction();
            $data = $request->all();
            $query = Rated::find($id);
