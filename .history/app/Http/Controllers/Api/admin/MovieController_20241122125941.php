    ...
    public function destroy($id)
{
    ...
    /**
 * @OA\Get(
 *     path="/api/admin/movieBygenre/{genre_id?}",
 *     tags={"Admin Movies"},
 *     summary="Get movies by genre or all movies if no genre is provided",
 *     operationId="getMovieByGenre",
 *     @OA\Parameter(
 *         name="genre_id",
 *         in="path",
 *         description="ID of genre (optional). If not provided, all movies will be returned.",
 *         required=false, // Đây là thay đổi quan trọng
 *         @OA\Schema(type="integer")
 *     ),
 *     @OA\Response(
 *         response=200,
 ...
 */
    ...
    public function getMovieByGenre($genre_id = null)
    {
        // Nếu không có genre_id, trả về toàn bộ danh sách phim
        if (is_null($genre_id)) {
            $movies = Movie::with('movie_genre')->get();
        } else {
            // Nếu có genre_id, lọc danh sách phim theo thể loại
            $movies = Movie::with('movie_genre')->whereHas('genres', function ($query) use ($genre_id) {
