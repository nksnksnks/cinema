...
class ThongkeController extends Controller
{
    ...
    private function getMonthlyRevenue($cinema_id, $startOfMonth, $endOfMonth)
    {
        $monthlyRevenue = DB::table('ci_bill')
            ->when($cinema_id != 0, fn ($query) => $query->when($cinema_id != 0, fn ($query) => $query->where('cinema_id', $cinema_id)))
    ...
    private function getNewUsersThisMonth($cinema_id,$startOfMonth,$endOfMonth)
    {
        ...
        $newUsers = DB::table('ci_account')
            ->when($cinema_id != 0, fn ($query) => $query->where('cinema_id', $cinema_id)) // Lọc theo cinema_id (Giả sử bảng ci_account có cột cinema_id)
            ->whereBetween('created_at', [$startOfMonth, $endOfMonth])
            ->count();
