...
class WeeklyTicketPriceController extends Controller
{
    ...
    {
        $specialdays = WeeklyTicketPrice::all();
        return view('admin.weekly_ticket_pricesindex', compact('specialdays'));
    }
    public function specialdayCreate()
    {
        $config['method'] = 'create';
        return view('admin.weekly_ticket_pricescreate', compact('config'));
    }
    public function specialdayStore(WeeklyTicketPriceRequest $request)
    {
        try {
            DB::beginTransaction();
            $data = $request->all();
            ...
            DB::commit();
            return redirect()
                ->route('weekly_ticket_pricescreate')
    ...
    {
        $specialday = WeeklyTicketPrice::find($id);
        $config['method'] = 'edit';
        return view('admin.weekly_ticket_pricescreate', compact('config', 'specialday'));
    }

    public function specialdayUpdate($id, WeeklyTicketPriceRequest $request)
    {
        try {
            DB::beginTransaction();
            $data = $request->all();
            ...
            DB::commit();
            return redirect()
                ->route('weekly_ticket_pricesindex')
                ->with('success', trans('messages.success.success'));
        } catch (\Throwable $th) {
            DB::rollBack();
            return redirect()
