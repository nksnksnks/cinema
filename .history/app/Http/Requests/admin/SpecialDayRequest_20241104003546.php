    ...
    public function rules()
    {
        ...
        switch ($method) {
            case 'POST': // Rules for creating a new weekly ticket price
                return [
                    'name' => 'required|string|max:255|unique:weekly_ticket_prices,name',
                    'description' => 'nullable|string|max:500',
                    'extra_fee' => 'required|integer|min:0',
                ];
            case 'PUT': // Rules for updating an existing weekly ticket price
                return [
                    'name' => 'required|string|max:255|unique:weekly_ticket_prices,name,' . $this->route('weekly_ticket_price')->id,
                    'description' => 'nullable|string|max:500',
                    'extra_fee' => 'required|integer|min:0',
                ];
            default:
