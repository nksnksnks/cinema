...
<table class="table table-striped table-bordered" id="table-movie">
    ...
    <tr>
        ...
        <td>
            {{ $weeklyticketprice->extra_fee }}
        </td>
        
        <td class="text-center">
            <a href="{{ route('weeklyticketprice.edit', $weeklyticketprice->id) }}" class="btn btn-success" ><i class="fa fa-edit"></i></a>
            <form action="{{ route('weeklyticketprice.destroy', $weeklyticketprice->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Bạn có chắc chắn muốn xóa không?')">
                @csrf
                @method('DELETE')
